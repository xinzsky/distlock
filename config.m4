...
PHP_ARG_ENABLE(distlock, whether to enable distlock support,
[  --enable-distlock               Enable distlock support])
...
if test "$PHP_DISTLOCK" != "no"; then
  ...
  if test "$PHP_LIBZOOKEEPER_DIR" = "no"; then
    ...
    PHP_ADD_LIBRARY_WITH_PATH(zookeeper_mt, $PHP_LIBZOOKEEPER_DIR/lib, DISTLOCK_SHARED_LIBADD)

    PHP_SUBST(DISTLOCK_SHARED_LIBADD)
    PHP_NEW_EXTENSION(distlock, zoo_lock.c php_distlock.c, $ext_shared)
