...
ZOOAPI int zkr_lock_init(zkr_lock_mutex_t* mutex, zhandle_t* zh,
    ...
    pthread_mutex_init(&(mutex->cond_mutex), NULL); // xinz+
    pthread_cond_init(&(mutex->cond),NULL);         // xinz+
    return 0;
}
...
static char* child_floor(char **sorted_data, int len, char *element) {
    ...
    int i =0;
    for (i=0; i < len; i++) {
        if (strcmp(strrchr(sorted_data[i],'-')+1, strrchr(element,'-')+1) < 0) { //xinz*: strcmp(sorted_data[i], element)
            ret = sorted_data[i];
        }
    }
    return ret;
...
static void lock_watcher_fn(zhandle_t* zh, int type, int state,
    ...
    //should be called
    zkr_lock_mutex_t *mutex = watcherCtx;
    
    zkr_lock_lock((zkr_lock_mutex_t*) watcherCtx, 0);
        ...
        if (mutex->id != NULL) {
            ...
            if (lessthanme != NULL) {
                ...
                if (strcmp(mutex->id, owner_id) == 0) {
                    ...
                    free_String_vector(vector); // xinz+,avoid memory leak.
                    return ZOK;
                }
            }
            free_String_vector(vector);
...
ZOOAPI int zkr_lock_lock(zkr_lock_mutex_t *mutex, int timeout) {
    pthread_mutex_lock(&(mutex->pmutex));
    zhandle_t *zh = mutex->zh;
    char *path = mutex->path;
    struct Stat stat;
    ...
    if(timeout != 0) {
      pthread_mutex_lock(&(mutex->cond_mutex));
      if(!zkr_lock_isowner(mutex)) { // if lock failure, block or timeout.
        if(timeout > 0) {
          struct timeval now;
          ...
          pthread_cond_timedwait(&(mutex->cond),&(mutex->cond_mutex), &tmout);
        } else {
...
ZOOAPI int zkr_lock_destroy(zkr_lock_mutex_t* mutex) {
    ...
    pthread_mutex_destroy(&(mutex->cond_mutex)); //xinx+
    pthread_cond_destroy(&(mutex->cond));        //xinz+
    mutex->isOwner = 0;
    if (mutex->ownerid) 
        free(mutex->ownerid);
    return 0;
